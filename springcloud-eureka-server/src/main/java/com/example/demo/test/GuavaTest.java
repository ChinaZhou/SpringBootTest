...
public class GuavaTest {
    ...
    Map<String, Integer> map2 = new HashMap<>(); //线程不安全
    ReentrantReadWriteLock lock = new ReentrantReadWriteLock(); //为map2增加并发锁

    ConcurrentHashMap<String, Integer> map3 = new ConcurrentHashMap<>(); //线程安全，但也要注意使用方式

    ConcurrentHashMap<String, AtomicLong> map4 = new ConcurrentHashMap<>();
    ...
    private void test(){
        ...
        System.out.println("#### ConcurrentHashMap<String, AtomicLong> #####");
        ...
        public void run() {
            try {
                for(int i=0; i<count; i++){
                    ...
                    map4.putIfAbsent(key, new AtomicLong(0));
                    map4.get(key).incrementAndGet();
